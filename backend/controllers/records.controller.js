...
async function list(req, res) {
  try {
    ...
    if (date_from) {
      if (!dateRe.test(date_from)) return res.status(400).json({ error: 'Invalid date_from format — expected YYYY-MM-DD' });
      where.push(`r.date >= $${pi++}`); params.push(date_from);
    }
    if (date_to) {
      if (!dateRe.test(date_to)) return res.status(400).json({ error: 'Invalid date_to format — expected YYYY-MM-DD' });
      where.push(`r.date <= $${pi++}`); params.push(date_to);
    }
    if (q) {
...
async function stats(req, res) {
  try {
    ...
    const [total, todayCount, activeGold, activeFD, closedGold,
      ...
      pool.query(`SELECT COUNT(*) FROM records WHERE is_deleted=FALSE AND date=$1`, [targetDate]),
      ...
      pool.query(`SELECT COUNT(*) FROM records WHERE is_deleted=FALSE AND section='gold' AND status='active' AND date=$1`, [targetDate]),
      pool.query(`SELECT COUNT(*) FROM records WHERE is_deleted=FALSE AND section='gold' AND status='closed' AND closed_date=$1`, [targetDate]),
...
async function getOne(req, res) {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT id, date, name, customer_id, customer_type, aadhar, mobile, account_no, section, tx_types, data, remarks, sonar_parent_no, sonar_sub_no, sonar_group_no, status, closed_date, closed_remarks, closed_tx_types, created_at, updated_at FROM records WHERE id=$1 AND is_deleted=FALSE LIMIT 1`, [req.params.id],
    );
  ...
  try {
    ...
    const { rows } = await pool.query(
      ...
      [date, name, customer_id || null, customer_type,
       aadhar || '', mobile || '', account_no || null,
       section, txStr, typeof data === 'object' ? JSON.stringify(data) : (data || '{}'), remarks || '',
  ...
  try {
    ...
    const result = await pool.query(
      ...
      [date, name, customer_id || null, customer_type || 'regular',
       aadhar || '', mobile || '',
       account_no || null, section, txStr, data || {}, remarks || '',
...
async function close(req, res) {
  try {
    ...
    const closeResult = await pool.query(
      `UPDATE records SET status='closed', closed_date=$1, closed_remarks=$2, closed_tx_types=$3, updated_at=NOW() WHERE id=$4 AND is_deleted=FALSE`,
      [closed_date || getTodayStr(), closed_remarks || '', closedTxStr, req.params.id],
    );
...
async function reopen(req, res) {
  try {
    const reopenResult = await pool.query(
      `UPDATE records SET status='active', closed_date=NULL, closed_remarks=NULL, updated_at=NOW() WHERE id=$1 AND is_deleted=FALSE`,
      [req.params.id],
    );
      ...
      try {
        ...
        const { rows } = await client.query(
          ...
          [rec.date || null, rec.name || null, rec.customer_id || null,
           ...
           rec.aadhar || '', rec.mobile || '', rec.account_no || null,
           rec.section || 'general', txStr, typeof rec.data === 'object' ? JSON.stringify(rec.data) : (rec.data || '{}'), rec.remarks || ''],
        );
  ...
  try {
    ...
    if ((txArr.includes('Saving Deposit') || txArr.includes('Saving Withdrawal')) && data.saving_acc_no) {
      try {
        ...
        // Guard: don't allow balance to go below zero on withdrawal
        if (delta < 0) {
          await pool.query(
...
async function upsertCustomer({ customer_id, name, aadhar, mobile, data = {} }) {
  try {
    ...
    if (customer_id) {
      ...
      if (!matchKey) return; // no usable key — skip silently
